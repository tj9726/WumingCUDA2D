module app
  ...
  real(8), allocatable, public :: nvector(:, :, :, :)
  real(8), allocatable, public :: ttensor(:, :, :, :)
  real(8) :: r(nsp)
  real(8) :: q(nsp)
  real(8) :: delt
  real(8) :: b0
...
contains
  ...
  subroutine app__main()
    ...
    do it = it0 + 1, max_it
      ...
      call field__fdtd_i(uf, up, gp, cumcnt, nxs, nxe, bc__dfield, bc__curre, bc__phi)
  ...
  subroutine init()
    ...
    allocate (nvector(4, nxgs - 1:nxge + 1, nys - 1:nye + 1, nsp))
    allocate (ttensor(10, nxgs - 1:nxge + 1, nys - 1:nye + 1, nsp))

!$OMP PARALLEL WORKSHARE
    np2(nys:nye, 1:nsp) = 0
    cumcnt(nxgs:nxge + 1, nys:nye, 1:nsp) = 0
    uf(1:6, nxgs - 2:nxge + 2, nys - 2:nye + 2) = 0.0d0
    up(1:ndim, 1:np, nys:nye, 1:nsp) = 0.0d0
    gp(1:ndim, 1:np, nys:nye, 1:nsp) = 0.0d0
...
!$OMP END PARALLEL WORKSHARE
    ...
    r(1) = mass_ratio
    r(2) = 1.0d0
    q(1) = sqrt(gam0 * r(1) / (4.0d0 * pi * n0)) * wpi
    q(2) = -sqrt(gam0 * r(2) / (4.0d0 * pi * n0)) * wpe
  ...
  subroutine set_initial_condition()
    ...
    do j = nys, nye
      do ii = 1, np2(j, isp)
        up(1, ii, j, 1) = (nxs + (nxe - nxs) * (ii - 5.0d-1) / np2(j, isp)) * delx
    ...
    do isp = 1, nsp
      ...
      do j = nys, nye
        do ii = 1, np2(j, isp)
          ...
          gam1 = 1.0d0 / sqrt(1.0d0 - v1 * v1 / (c * c))
          gamp = sqrt(1.0d0 + (up(3, ii, j, isp) * up(3, ii, j, isp) &
  ...
  subroutine relocate()
    ...
    do j = nys, nye
      do ii = 1, n0
        ...
        up(1, ii1, j, 1) = (nxe - 1) * delx + (ii - 5.0d-1) / n0 * delx
  ...
  subroutine inject()
    ...
    do j = nys, nye
      do ii = 1, nlinj_grid(j)
        ...
        up(1, ii1, j, 1) = nxe * delx + (ii - 5.0d-1) / nlinj_grid(j) * x0
        up(2, ii1, j, 1) = (j + uniform_rand()) * delx
        up(1, ii2, j, 2) = up(1, ii1, j, 1)
        up(2, ii2, j, 2) = up(2, ii1, j, 1)
      end do
