...
contains
  ...
  !
  subroutine paraio__mom(nvector, ttensor, uf, it)
    implicit none
    integer, intent(in) :: it
    real(8), intent(in) :: uf(6, nxgs - 2:nxge + 2, nys - 2:nye + 2)
    real(8), intent(in) :: nvector(4, nxgs - 1:nxge + 1, nys - 1:nye + 1, nsp), ttensor(10, nxgs - 1:nxge + 1, nys - 1:nye + 1, nsp)
    ...
    ! nvector
    nd = 4
    lshape = (/4, nxg, nyl, nsp/)
    gshape = (/4, nxg, nyg, nsp/)
    offset = (/0, 0, nys * nrank, 0/)
    lsize = product(lshape(1:nd))
    gsize = product(gshape(1:nd))
    dsize = gsize * 8
    desc = "particle flux vector"
    mpibuf1(1:lsize) = reshape(nvector(1:4, nxgs:nxge, nys:nye, 1:nsp), (/lsize/))
    call jsonio_put_metadata(json, p, 'nvector', 'f8', disp, dsize, nd, gshape, desc)
    call mpiio_write_collective(fh, disp, nd, gshape, lshape, offset, mpibuf1)

    ! ttensor
    nd = 4
    lshape = (/10, nxg, nyl, nsp/)
    gshape = (/10, nxg, nyg, nsp/)
    offset = (/0, 0, nys * nrank, 0/)
    lsize = product(lshape(1:nd))
    gsize = product(gshape(1:nd))
    dsize = gsize * 8
    desc = "stress-energy tensor"
    mpibuf1(1:lsize) = reshape(ttensor(1:10, nxgs:nxge, nys:nye, 1:nsp), (/lsize/))
    call jsonio_put_metadata(json, p, 'ttensor', 'f8', disp, dsize, nd, gshape, desc)
    call mpiio_write_collective(fh, disp, nd, gshape, lshape, offset, mpibuf1)
    ...
    call jsonio_put_metadata(json, p, 'uf', 'f8', disp, dsize, nd, gshape, desc)
    call mpiio_write_collective(fh, disp, nd, gshape, lshape, offset, mpibuf1)
