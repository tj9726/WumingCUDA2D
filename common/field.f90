...
contains
  ...
  subroutine field__init(ndim_in, np_in, nsp_in, nxgs_in, nxge_in, nygs_in, nyge_in, nys_in, nye_in, &
    ...
    f2 = gfac * f1 * f1
    f3 = 4.0d0 * pi * delx / c
    f4 = 4.0d0 + (delx / (c * delt * gfac)) * (delx / (c * delt * gfac))
    f5 = (delx / (c * delt * gfac)) * (delx / (c * delt * gfac))
    d_delx = 1.0d0 / delx
    d_delt = 1.0d0 / delt
